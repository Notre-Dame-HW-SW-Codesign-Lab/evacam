...
#define INITIAL_BASIC_WIRE { \
	...
	localWire->Initialize(inputParameter->processNode, basicWireType, basicWireRepeaterType, inputParameter->temperature, isBasicLowSwing, inputParameter); \
	...
	if (inputParameter->minIsGlobalWireLowSwing == inputParameter->maxIsGlobalWireLowSwing) \
		...
		isBasicLowSwing = false; \
	globalWire->Initialize(inputParameter->processNode, basicWireType, basicWireRepeaterType, inputParameter->temperature, isBasicLowSwing, inputParameter); \
}
...
#define REFINE_LOCAL_WIRE_FORLOOP \
	...
	for (localWireepeaterType = inputParameter->minLocalWireRepeaterType; localWireepeaterType <= inputParameter->maxLocalWireRepeaterType; localWireepeaterType++) \
	...
	if ((WireRepeaterType)localWireepeaterType == repeated_none || (bool)isLocalWireLowSwing == false)
...
#define REFINE_GLOBAL_WIRE_FORLOOP \
	...
	for (globalWireepeaterType = inputParameter->minGlobalWireRepeaterType; globalWireepeaterType <= inputParameter->maxGlobalWireRepeaterType; globalWireepeaterType++) \
	...
	if ((WireRepeaterType)globalWireepeaterType == repeated_none || (bool)isGlobalWireLowSwing == false)
...
#define TRY_AND_UPDATE(oldResult, memoryType) { \
	if (inputParameter->routingMode == h_tree) \
		trialBank = make_shared<BankWithHtree>(); \
	else \
		trialBank = make_shared<BankWithoutHtree>(); \
	trialBank->Initialize((oldResult).bank->numRowMat, (oldResult).bank->numColumnMat, (oldResult).bank->capacity, (oldResult).bank->blockSize, (oldResult).bank->associativity, \
				...
				(oldResult).bank->numActiveSubarrayPerRow, (oldResult).bank->numActiveSubarrayPerColumn, (oldResult).bank->areaOptimizationLevel, (memoryType), cell->camType, inputParameter->searchFunction); \
...
#define CALCULATE(bank, memoryType) { \
	...
	(bank)->Initialize(numRowMat, numColumnMat, capacity, blockSize, associativity, \
				...
				numActiveSubarrayPerRow, numActiveSubarrayPerColumn, (BufferDesignTarget)areaOptimizationLevel, (memoryType), cell->camType, inputParameter->searchFunction); \
	(bank)->CalculateArea(); \
	(bank)->CalculateRC(); \
...
#define VERIFY_DATA_CAPACITY { \
	if ((long long)dataBank->mat->subarray->numColumn * dataBank->mat->subarray->numRow * dataBank->numColumnMat * \
			dataBank->numRowMat * dataBank->numColumnSubarray * dataBank->numRowSubarray != capacity) { \
				std::cout << "numcolumn x numrow x numcolumnmat x numrowmat x numcolumnsubarry x numrowsubarray" << dataBank->mat->subarray->numColumn  << ": " << dataBank->mat->subarray->numRow   << ": " << dataBank->numColumnMat << ": " << \
			dataBank->numRowMat << ": " <<  dataBank->numColumnSubarray << ": " <<  dataBank->numRowSubarray << std::endl; \
...
// Turned into a method of InputParameter, not a macro
// Also, this is unused
...
// Turned into a method of InputParameter
...
// New macro for all non exiting errors/warnings for cleaner output without providing the verbose flag
