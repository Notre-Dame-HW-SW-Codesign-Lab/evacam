...
\node[class, fill=blue!8, below=2.0cm of config, text width=4.8cm] (techctx) {%
  \textbf{TechnologyContext}\\
  + tech: Technology\\
  + fefetTech: Technology\\
  + cell: MemCell
};

\node[class, fill=blue!8, right=4.0cm of techctx, text width=4.8cm] (techloader) {%
...
\draw[owns] (config) -- node[left] {\scriptsize owns} (techctx);
\draw[owns] (techctx) -- node[above] {\scriptsize owns} (tech);
\draw[owns] (techctx) -- node[above] {\scriptsize owns} (memcell);
\draw[uses] (config) -- node[above] {\scriptsize loads via} (techloader);
...
% Package outlines
\node[package, fit=(config)(tech)(memcell)(techctx)(techloader), label=above:{\textbf{Configuration Model}}] {};
