...
void SubArray::Initialize(long long _numRow, long long _numColumn, bool _multipleRowPerSet, bool _split,
		...
		BufferDesignTarget _areaOptimizationLevel, std::shared_ptr<InputParameter> _inputParameter,
	...
	if (inputParameter->cell->memCellType == DRAM || inputParameter->cell->memCellType == eDRAM) {
		if (muxSenseAmp > 1) {
	...
	if (inputParameter->cell->memCellType == SLCNAND) {
	...
	if (inputParameter->cell->memCellType == memristor || inputParameter->cell->memCellType == FBRAM) {
		if (internalSenseAmp && muxSenseAmp < 2) {
			/* There is no way to put the sense amp */
			invalid = true;
	...
	if (inputParameter->cell->memCellType == FBRAM) {
		if (inputParameter->cell->resistanceOff / inputParameter->cell->resistanceOn < numRow / BITLINE_LEAKAGE_TOLERANCE) {
			/* bitline too long */
			invalid = true;
		...
		maxBitlineCurrent = MAX(inputParameter->cell->resetCurrent, inputParameter->cell->setCurrent) + inputParameter->cell->leakageCurrentAccessDevice * (numRow - 1);
	}

	if (inputParameter->cell->memCellType == MRAM || inputParameter->cell->memCellType == PCRAM || inputParameter->cell->memCellType == memristor) {
		if (inputParameter->cell->accessType == CMOS_access){
			if (inputParameter->tech->currentOnNmos[inputParameter->temperature - 300]
									/ inputParameter->tech->currentOffNmos[inputParameter->temperature - 300] < numRow / BITLINE_LEAKAGE_TOLERANCE) {
				/* bitline too long */
				invalid = true;
				std::cout << "bitline too long" << std::endl;
				initialized = true;
			...
			double resetCurrent;
			if (inputParameter->cell->resetCurrent == 0) {
				resetCurrent = (fabs (inputParameter->cell->resetVoltage) - inputParameter->cell->voltageDropAccessDevice) / inputParameter->cell->resistanceOnAtResetVoltage;
			} else
				resetCurrent = inputParameter->cell->resetCurrent;
			int numSelectedColumnPerRow = numColumn / muxSenseAmp / muxOutputLev1 / muxOutputLev2;
			if (inputParameter->cell->accessType == none_access) {
				maxWordlineCurrent = resetCurrent * numSelectedColumnPerRow + resetCurrent * inputParameter->cell->resistanceOnAtResetVoltage
						/ 2 / inputParameter->cell->resistanceOnAtHalfResetVoltage * (numColumn - numSelectedColumnPerRow);
			} else { //diode or BJT
				maxWordlineCurrent = resetCurrent * numSelectedColumnPerRow + inputParameter->cell->leakageCurrentAccessDevice
						* (numColumn - numSelectedColumnPerRow);
			}
			double minWordlineDriverWidth = maxWordlineCurrent / inputParameter->tech->currentOnNmos[inputParameter->temperature - 300];
			if (minWordlineDriverWidth > inputParameter->maxNmosSize * inputParameter->tech->featureSize) {
				invalid = true;
				std::cout << "too large minWordlineDriverWidth" << std::endl;
				return;
			}
			if (inputParameter->cell->accessType == none_access) {
				maxBitlineCurrent = resetCurrent + resetCurrent * inputParameter->cell->resistanceOnAtResetVoltage / 2
						/ inputParameter->cell->resistanceOnAtHalfResetVoltage * (numRow - 1);
			} else { //diode or BJT
				maxBitlineCurrent = resetCurrent + inputParameter->cell->leakageCurrentAccessDevice * (numRow - 1);
			}
		}
	}

	double minBitlineMuxWidth = maxBitlineCurrent / inputParameter->tech->currentOnNmos[inputParameter->temperature - 300];
	minBitlineMuxWidth = MAX(MIN_NMOS_SIZE * inputParameter->tech->featureSize, minBitlineMuxWidth);
	if (minBitlineMuxWidth > inputParameter->maxNmosSize * inputParameter->tech->featureSize) {
		invalid = true;
		std::cout << "minBitlineMuxWidth > inputParameter->maxNmosSize * inputParameter->tech->featureSize" << std::endl;
		return;
	}

	if (internalSenseAmp) {
		if (inputParameter->cell->memCellType == SRAM || inputParameter->cell->memCellType == DRAM || inputParameter->cell->memCellType == eDRAM) {
			/* SRAM, DRAM, and eDRAM all use voltage sensing */
			voltageSense = true;
		} else if (inputParameter->cell->memCellType == MRAM || inputParameter->cell->memCellType == PCRAM || inputParameter->cell->memCellType == memristor || inputParameter->cell->memCellType == FBRAM) {
			voltageSense = inputParameter->cell->readMode;
		} else {/* NAND flash */
			voltageSense = true;
		}
	} else if (inputParameter->cell->memCellType == DRAM || inputParameter->cell->memCellType == eDRAM) {
		std::cout << "[Subarray] Error: DRAM does not support external sense amplifiers!" << std::endl;
		exit(-1);
	}

	if (inputParameter->cell->memCellType == DRAM || inputParameter->cell->memCellType == eDRAM) {
		senseVoltage = inputParameter->tech->vdd / 2 * inputParameter->cell->capDRAMCell / (inputParameter->cell->capDRAMCell + capBitline);
		if (senseVoltage < inputParameter->cell->minSenseVoltage) {		/* Bitline is too long */
			invalid = true;
		...
		senseVoltage = MAX(inputParameter->cell->minSenseVoltage, 0.2 * inputParameter->tech->vdd);
	} else {
		/* TODO: different memory inputParameter->technology might have different values here */
		senseVoltage = inputParameter->cell->minSenseVoltage;
	}

	/* Derived parameters */
	numSenseAmp = numColumn / muxSenseAmp;
	lenWordline = (double)numColumn * inputParameter->cell->widthInFeatureSize * inputParameter->tech->featureSize;
	lenBitline = (double)numRow * inputParameter->cell->heightInFeatureSize * inputParameter->tech->featureSize;
	/* Add stitching overhead if necessary */
	if (inputParameter->cell->stitching) {
		lenWordline += ((numColumn - 1) / inputParameter->cell->stitching + 1) * STITCHING_OVERHEAD * inputParameter->tech->featureSize;
	}
	...
	if (inputParameter->cell->memCellType == SLCNAND) {
		...
		lenBitline += (numRow / pageCount) * 5 * inputParameter->tech->featureSize;
	}
	...
	double capMuxLoad, resMuxLoad;
        ...
        capMuxLoad = CalculateGateCap(minBitlineMuxWidth, inputParameter->tech) * numColumn;
	...
	if (inputParameter->cell->memCellType == SRAM) {
		...
		resCellAccess = CalculateOnResistance(inputParameter->cell->widthAccessCMOS * inputParameter->tech->featureSize, NMOS, inputParameter->temperature, inputParameter->tech);
		capCellAccess = CalculateDrainCap(inputParameter->cell->widthAccessCMOS * inputParameter->tech->featureSize, NMOS, inputParameter->cell->widthInFeatureSize * inputParameter->tech->featureSize, inputParameter->tech);
		capWordline += 2 * CalculateGateCap(inputParameter->cell->widthAccessCMOS * inputParameter->tech->featureSize, inputParameter->tech) * numColumn;
		capBitline  += capCellAccess * numRow / 2;	/* Due to shared contact */
		voltagePrecharge = inputParameter->tech->vdd / 2;	/* SRAM read voltage is always half of vdd */
	} else if (inputParameter->cell->memCellType == DRAM || inputParameter->cell->memCellType == eDRAM) {
		/* DRAM and eDRAM only has one access transistors */
		resCellAccess = CalculateOnResistance(inputParameter->cell->widthAccessCMOS * inputParameter->tech->featureSize, NMOS, inputParameter->temperature, inputParameter->tech);
		capCellAccess = CalculateDrainCap(inputParameter->cell->widthAccessCMOS * inputParameter->tech->featureSize, NMOS, inputParameter->cell->widthInFeatureSize * inputParameter->tech->featureSize, inputParameter->tech);
		capWordline += CalculateGateCap(inputParameter->cell->widthAccessCMOS * inputParameter->tech->featureSize, inputParameter->tech) * numColumn;
		capBitline  += capCellAccess * numRow / 2;	/* Due to shared contact */
		voltagePrecharge = inputParameter->tech->vdd / 2;	/* DRAM read voltage is always half of vdd */
	} else if (inputParameter->cell->memCellType == FBRAM) { /* Floating Body RAM */
		resCellAccess = 0;
		capCellAccess = CalculateFBRAMDrainCap(inputParameter->cell->widthSOIDevice * inputParameter->tech->featureSize, inputParameter->tech);
		capWordline += CalculateFBRAMGateCap(inputParameter->cell->widthSOIDevice * inputParameter->tech->featureSize, inputParameter->cell->gateOxThicknessFactor, inputParameter->tech) * numColumn;
		capBitline  += capCellAccess * numRow / 2;	/* Due to shared contact */
		resMemCellOff = inputParameter->cell->resistanceOff;
		resMemCellOn = inputParameter->cell->resistanceOn;
		if (inputParameter->cell->readMode) {						/* voltage-sensing */
			if (inputParameter->cell->readVoltage == 0) {  /* Current-in voltage sensing */
				voltageMemCellOff = inputParameter->cell->readCurrent * resMemCellOff;
				voltageMemCellOn = inputParameter->cell->readCurrent * resMemCellOn;
				voltagePrecharge = (voltageMemCellOff + voltageMemCellOn) / 2;
				voltagePrecharge = MIN(inputParameter->tech->vdd, voltagePrecharge);  /* TODO: we can have charge bump to increase SA working point */
				...
				voltageMemCellOff = inputParameter->cell->readVoltage * resMemCellOff / (resMemCellOff + resInSerialForSenseAmp);
				voltageMemCellOn = inputParameter->cell->readVoltage * resMemCellOn / (resMemCellOn + resInSerialForSenseAmp);
				voltagePrecharge = (voltageMemCellOff + voltageMemCellOn) / 2;
				voltagePrecharge = MIN(inputParameter->tech->vdd, voltagePrecharge);  /* TODO: we can have charge bump to increase SA working point */
				if ((voltagePrecharge - voltageMemCellOn) <= senseVoltage) {
		...
		if (inputParameter->cell->accessType == CMOS_access) {
			...
			capWordline += CalculateGateCap(inputParameter->cell->widthAccessCMOS * inputParameter->tech->featureSize, inputParameter->tech) * numColumn;
			capBitline  += capCellAccess * numRow / 2;	/* Due to shared contact */
		} else if (inputParameter->cell->accessType == BJT_access) {
			// TODO
	/*	} else if (inputParameter->cell->accessType == diode_access){
			if (inputParameter->cell->readVoltage == 0) {
				resCellAccess = inputParameter->cell->voltageDropAccessDevice / inputParameter->cell->readCurrent;
			} else {
				if (inputParameter->cell->readMode == false) {
					resCellAccess = inputParameter->cell->voltageDropAccessDevice / (inputParameter->cell->readVoltage
							- inputParameter->cell->voltageDropAccessDevice) * inputParameter->cell->resistanceOn;
				} else {
					std::cout<<"Error[Subarray]: Diode access do not support voltage-input voltage sensing" <<std::endl;
					exit(-1);
				}
			}
			capCellAccess = MAX(inputParameter->cell->capacitanceOn, inputParameter->cell->capacitanceOff);
			capWordline += MAX(inputParameter->cell->capacitanceOff, inputParameter->cell->capacitanceOn) * numColumn;
			capBitline += MAX(inputParameter->cell->capacitanceOff, inputParameter->cell->capacitanceOn) * numRow;      */
		} else { // none_access || diode_access
			// resCellAccess = 0;
			// capCellAccess = MAX(inputParameter->cell->capacitanceOn, inputParameter->cell->capacitanceOff);
			// capWordline += MAX(inputParameter->cell->capacitanceOff, inputParameter->cell->capacitanceOn) * numColumn;  //TODO: choose the right capacitance
			// capBitline += MAX(inputParameter->cell->capacitanceOff, inputParameter->cell->capacitanceOn) * numRow;      //TODO: choose the right capacitance
		}
		resMemCellOff = resCellAccess + inputParameter->cell->resistanceOff;
		resMemCellOn = resCellAccess + inputParameter->cell->resistanceOn;
		if (inputParameter->cell->readMode) {						/* voltage-sensing */
			if (inputParameter->cell->readVoltage == 0) {  /* Current-in voltage sensing */
				voltageMemCellOff = inputParameter->cell->readCurrent * resMemCellOff;
				voltageMemCellOn = inputParameter->cell->readCurrent * resMemCellOn;
		...
		resCellAccess = CalculateOnResistance(inputParameter->tech->featureSize, NMOS, inputParameter->temperature, inputParameter->tech) * stringLength;
		capCellAccess = CalculateDrainCap(inputParameter->tech->featureSize, NMOS, inputParameter->cell->widthInFeatureSize * inputParameter->tech->featureSize, inputParameter->tech);
		/* The capacitance of each cell at the gate terminal is the series of C_control_gate | C_floating_gate */
		capWordline += CalculateGateCap(inputParameter->tech->featureSize, inputParameter->tech) * numColumn * inputParameter->cell->gateCouplingRatio / (inputParameter->cell->gateCouplingRatio + 1);
		capBitline  += capCellAccess * (numRow / pageCount) / 2;	/* 2 is due to shared contact and the effective row count is numRow/pageCount */
		voltagePrecharge = inputParameter->tech->vdd * 0.6;	/* SLC NAND flash bitline precharge voltage is assumed to 0.6Vdd */
	} else {	/* MLC NAND flash */
		// TODO
	}
	...
	precharger->Initialize(inputParameter->tech->vdd, numColumn, capBitline, resBitline, inputParameter, localWire);
	//precharger->CalculateRC();

	rowDecoder->Initialize(numRow, capWordline, resWordline, multipleRowPerSet, areaOptimizationLevel, maxWordlineCurrent, inputParameter);
	...
	if (!invalid) {
		bitlineMuxDecoder->Initialize(muxSenseAmp, capMuxLoad, resMuxLoad /* TODO: need to fix */, false, areaOptimizationLevel, 0, inputParameter);
	...
	if (!invalid) {
		senseAmpMuxLev1Decoder->Initialize(muxOutputLev1, capMuxLoad, resMuxLoad /* TODO: need to fix */, false, areaOptimizationLevel, 0, inputParameter);
	...
	if (!invalid) {
		senseAmpMuxLev2Decoder->Initialize(muxOutputLev2, capMuxLoad, resMuxLoad /* TODO: need to fix */, false, areaOptimizationLevel, 0, inputParameter);
	...
	senseAmpMuxLev2->Initialize(muxOutputLev2, numColumn / muxSenseAmp / muxOutputLev1 / muxOutputLev2, 0, 0 /* TODO: need to fix */, maxBitlineCurrent, inputParameter);
	//senseAmpMuxLev2->CalculateRC();

	senseAmpMuxLev1->Initialize(muxOutputLev1, numColumn / muxSenseAmp / muxOutputLev1,
			senseAmpMuxLev2->capForPreviousDelayCalculation, senseAmpMuxLev2->capForPreviousPowerCalculation, maxBitlineCurrent, inputParameter);
	//senseAmpMuxLev1->CalculateRC();

	if (internalSenseAmp) {
		if (!invalid) {
			senseAmp->Initialize(numSenseAmp, !voltageSense, senseVoltage, lenWordline / numColumn * muxSenseAmp, inputParameter);
			if (senseAmp->invalid){
		...
		if (!invalid) {
			bitlineMux->Initialize(muxSenseAmp, numColumn / muxSenseAmp, senseAmp->capLoad, senseAmp->capLoad, maxBitlineCurrent, inputParameter);
		}
	} else {
		if (!invalid) {
			bitlineMux->Initialize(muxSenseAmp, numColumn / muxSenseAmp,
					senseAmpMuxLev1->capForPreviousDelayCalculation, senseAmpMuxLev1->capForPreviousPowerCalculation, maxBitlineCurrent, inputParameter);
		}
	}
...
void SubArray::CalculateLatency(double _rampInput) {
	if (!initialized) {
		...
		if (inputParameter->cell->memCellType == SRAM) {
			...
			double resPullDown = CalculateOnResistance(inputParameter->cell->widthSRAMCellNMOS * inputParameter->tech->featureSize, NMOS,
			...
			double gm = CalculateTransconductance(inputParameter->cell->widthAccessCMOS * inputParameter->tech->featureSize, NMOS, inputParameter->tech);
			...
			double cap = (capCellAccess + inputParameter->cell->capDRAMCell) * (capBitline + bitlineMux->capForPreviousDelayCalculation)
					/ (capCellAccess + inputParameter->cell->capDRAMCell + capBitline + bitlineMux->capForPreviousDelayCalculation);
			...
			if (inputParameter->cell->memCellType == PCRAM) {
				if (inputParameter->writeScheme == write_and_verify) {
					...
					resetLatency = writeLatency + inputParameter->cell->resetPulse;
					setLatency = writeLatency + inputParameter->cell->setPulse;
					writeLatency += MAX(inputParameter->cell->resetPulse, inputParameter->cell->setPulse);
				}
			} else if (inputParameter->cell->memCellType == FBRAM) {
				writeLatency = MAX(rowDecoder->writeLatency, columnDecoderLatency + chargeLatency);
				resetLatency = writeLatency + inputParameter->cell->resetPulse;
				setLatency = writeLatency + inputParameter->cell->setPulse;
				writeLatency += MAX(inputParameter->cell->resetPulse, inputParameter->cell->setPulse);
			} else { //memristor and MRAM
				if (inputParameter->cell->accessType == diode_access || inputParameter->cell->accessType == none_access) {
					...
					writeLatency += inputParameter->cell->resetPulse + inputParameter->cell->setPulse;
			...
			int stringLength = pageCount + 2;
			/* Codes below calculate the bitline latency */
			double resPullDown = CalculateOnResistance(inputParameter->tech->featureSize, NMOS, inputParameter->temperature, inputParameter->tech)
					* stringLength;
			...
			double gm = CalculateTransconductance(inputParameter->tech->featureSize, NMOS, inputParameter->tech);	/* minimum size transistor */
			double beta = 1 / (resPullDown * gm);
			double bitlineRamp = 0;
			bitlineDelay = horowitz(tau, beta, rowDecoder->rampOutput, &bitlineRamp);
			...
			resetLatency = MAX(rowDecoder->readLatency, columnDecoderLatency + chargeLatency) + inputParameter->cell->flashEraseTime;
			/* calculate the programming time, a.k.a. set here */
			setLatency = MAX(rowDecoder->readLatency, columnDecoderLatency + chargeLatency) + inputParameter->cell->flashProgramTime;
...
void SubArray::CalculatePower() {
	if (!initialized) {
		...
		if (inputParameter->cell->memCellType == SRAM) {
			...
			readDynamicEnergy = (capCellAccess + capBitline + bitlineMux->capForPreviousPowerCalculation) * senseVoltage * inputParameter->tech->vdd * numColumn;
			double writeVoltage = inputParameter->cell->resetVoltage;	/* should also equal to setVoltage, for DRAM, it is Vdd */
			...
			if (inputParameter->cell->readMode == false) {	/* current-sensing */
				...
				double vpreMin = inputParameter->cell->readVoltage * resBitlineMux / (resBitlineMux + resBitline +resMemCellOn);
				double vpreMax = inputParameter->cell->readVoltage * (resBitlineMux + resBitline) / (resBitlineMux + resBitline + resMemCellOn);
			...
			if (inputParameter->cell->readPower == 0) 
				cellReadEnergy = 2 * inputParameter->cell->CalculateReadPower() * senseAmp->readLatency; /* x2 is because of the reference cell */
			else
				cellReadEnergy = 2 * inputParameter->cell->readPower * senseAmp->readLatency;
			...
			double resetEnergyPerBit = inputParameter->cell->resetEnergy;
			double setEnergyPerBit = inputParameter->cell->setEnergy;
			if (inputParameter->cell->setMode)
				setEnergyPerBit += (capCellAccess + capBitline + bitlineMux->capForPreviousPowerCalculation) * inputParameter->cell->setVoltage * inputParameter->cell->setVoltage;
			else
				setEnergyPerBit += (capCellAccess + capBitline + bitlineMux->capForPreviousPowerCalculation) * inputParameter->tech->vdd * inputParameter->tech->vdd;
			if (inputParameter->cell->resetMode)
				resetEnergyPerBit += (capCellAccess + capBitline + bitlineMux->capForPreviousPowerCalculation) * inputParameter->cell->resetVoltage * inputParameter->cell->resetVoltage;
			else
				resetEnergyPerBit += (capCellAccess + capBitline + bitlineMux->capForPreviousPowerCalculation) * inputParameter->tech->vdd * inputParameter->tech->vdd;
			...
			} else if (inputParameter->cell->memCellType == FBRAM){ //FBRAM write energy
				cellResetEnergy = resetEnergyPerBit * numColumn / muxSenseAmp / muxOutputLev1 / muxOutputLev2;
				cellSetEnergy = setEnergyPerBit * numColumn / muxSenseAmp / muxOutputLev1 / muxOutputLev2;
			...
			double actualWordlineReadEnergy = rowDecoder->readDynamicEnergy / inputParameter->tech->vdd / inputParameter->tech->vdd
					* inputParameter->cell->flashPassVoltage * inputParameter->cell->flashPassVoltage;	/* approximate calculate, the wordline is charged to Vpass instead of Vdd */
			...
			setDynamicEnergy = (capCellAccess + capBitline + bitlineMux->capForPreviousPowerCalculation)
					* inputParameter->cell->flashProgramVoltage * inputParameter->cell->flashProgramVoltage * numColumn;
			 ...
			 */
			setDynamicEnergy += DELTA_V_TH * TUNNEL_CURRENT_FLOW * inputParameter->cell->area
					* inputParameter->tech->featureSize * inputParameter->tech->featureSize * inputParameter->cell->flashProgramTime * numColumn;
			...
			actualWordlineSetEnergy += rowDecoder->setDynamicEnergy / inputParameter->tech->vdd / inputParameter->tech->vdd
					* inputParameter->cell->flashPassVoltage * inputParameter->cell->flashPassVoltage * (numRow / pageCount * stringLength - 1);
			/* And the selected wordline is precharged to Vpgm */
			actualWordlineSetEnergy += rowDecoder->setDynamicEnergy / inputParameter->tech->vdd / inputParameter->tech->vdd
					* inputParameter->cell->flashProgramVoltage * inputParameter->cell->flashProgramVoltage;
			...
			resetDynamicEnergy = (capCellAccess + capBitline + bitlineMux->capForPreviousPowerCalculation)
					* (inputParameter->cell->flashEraseVoltage - inputParameter->tech->buildInPotential) * (inputParameter->cell->flashEraseVoltage - inputParameter->tech->buildInPotential);
			...
			double wellJunctionCap = inputParameter->tech->capJunction * inputParameter->cell->area * inputParameter->tech->featureSize * inputParameter->tech->featureSize;
			wellJunctionCap *= inputParameter->flashBlockSize;	/* one block shares the same well */
			resetDynamicEnergy += wellJunctionCap * inputParameter->cell->flashEraseVoltage * inputParameter->cell->flashEraseVoltage;
			 ...
			 */
			double beta = 0.8;
			double actualWordlineResetEnergy = rowDecoder->resetDynamicEnergy / inputParameter->tech->vdd / inputParameter->tech->vdd
					* (inputParameter->cell->flashEraseVoltage * beta) * (inputParameter->cell->flashEraseVoltage * beta);
