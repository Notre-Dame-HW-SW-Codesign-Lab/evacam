...
void Result::Initialize(std::shared_ptr<InputParameter> _inputParameter) {
	...
	localWire = std::make_shared<Wire>();
	globalWire = std::make_shared<Wire>();
        
        // TODO: Added inits for wires from main, no idea if that's accurate for calculations,
        //       however these wires do need to be initialized
...
/*
	...
	outputFile << (bank->mat->subarray->bitlineMux->writeDynamicEnergy
					+ bank->mat->subarray->senseAmpMuxLev1->writeDynamicEnergy
					+ bank->mat->subarray->senseAmpMuxLev2->writeDynamicEnergy) * 1e12 << std::endl;
*/
	...
	outputFile << inputParameter->cell->area * inputParameter->tech->featureSize * inputParameter->tech->featureSize * bank->capacity / bank->area * 100 << ",";
