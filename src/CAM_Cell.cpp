...
void CAM_MemCell::ReadCellFromFile(const std::string & inputFile, DesignTarget _designTarget, double _vdd) {
        ...
        if (!inFile) throw std::runtime_error("Could not open memory cell configuration file.");
	...
	while (std::getline(inFile, line)) {
		...
		if (starts_with(line, "-MemCellType")) {
			...
			if (tmp == "SRAM")
                ...
                } else if (starts_with(line, "-CellArea")) {
			area = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-CellAspectRatio")) {
			aspectRatio = parse_double_after_colon(line);
			heightInFeatureSize = sqrt(area * aspectRatio);
			widthInFeatureSize = sqrt(area / aspectRatio);
			
		} else if (starts_with(line, "-ResistanceOnAtSetVoltage")) {
			resistanceOnAtSetVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ResistanceOffAtSetVoltage")) {
			resistanceOffAtSetVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ResistanceOnAtResetVoltage")) {
			resistanceOnAtResetVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ResistanceOffAtResetVoltage")) {
			resistanceOffAtResetVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ResistanceOnAtReadVoltage")) {
			resistanceOnAtReadVoltage = parse_double_after_colon(line);
			resistanceOn = resistanceOnAtReadVoltage;
			
		} else if (starts_with(line, "-ResistanceOffAtReadVoltage")) {
			resistanceOffAtReadVoltage = parse_double_after_colon(line);
			resistanceOff = resistanceOffAtReadVoltage;
			
		} else if (starts_with(line, "-ResistanceOnAtHalfReadVoltage")) {
			resistanceOnAtHalfReadVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ResistanceOffAtHalfReadVoltage")) {
			resistanceOffAtHalfReadVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ResistanceOnAtHalfResetVoltage")) {
			resistanceOnAtHalfResetVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ResistanceOn")) {
			resistanceOn = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ResistanceOff")) {
			resistanceOff = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-CapacitanceOn")) {
			capacitanceOn = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-CapacitanceOff")) {
			capacitanceOff = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-GateOxThicknessFactor")) {
			gateOxThicknessFactor = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-SOIDeviceWidth (F)")) {
			widthSOIDevice = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ReadMode")) {
			tmp = parse_string_after_colon(line);
			if (tmp == "voltage")
				readMode = true;
			else
		...
		} else if (starts_with(line, "-ReadVoltage")) {
			readVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ReadCurrent")) {
			readCurrent = parse_double_after_colon(line);
			readCurrent /= 1e6;
			
		} else if (starts_with(line, "-ReadPower")) {
			readPower = parse_double_after_colon(line);
			readPower /= 1e6;
			
		} else if (starts_with(line, "-WordlineBoostRatio")) {
			wordlineBoostRatio = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-MinSenseVoltage")) {
			minSenseVoltage = parse_double_after_colon(line);
			minSenseVoltage /= 1e3;
			
		} else if (starts_with(line, "-ResetMode")) {
			tmp = parse_string_after_colon(line);
			if (tmp == "voltage")
				resetMode = true;
			else
		...
		} else if (starts_with(line, "-ResetVoltage")) {
			resetVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ResetCurrent")) {
			resetCurrent = parse_double_after_colon(line);
			resetCurrent /= 1e6;
			
		} else if (starts_with(line, "-ResetVoltage")) {
			resetVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ResetPulse")) {
			resetPulse = parse_double_after_colon(line);
			resetPulse /= 1e9;
			
		} else if (starts_with(line, "-ResetEnergy")) {
			resetEnergy = parse_double_after_colon(line);
			resetEnergy /= 1e12;
			
		} else if (starts_with(line, "-SetMode")) {
			tmp = parse_string_after_colon(line);
			if (tmp == "voltage")
				setMode = true;
			else
		...
		} else if (starts_with(line, "-SetVoltage")) {
			setVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-SetCurrent")) {
			setCurrent = parse_double_after_colon(line);
			setCurrent /= 1e6;
			
		} else if (starts_with(line, "-SetVoltage")) {
			setVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-SetPulse")) {
			setPulse = parse_double_after_colon(line);
			setPulse /= 1e9;
			
		} else if (starts_with(line, "-SetEnergy")) {
			setEnergy = parse_double_after_colon(line);
			setEnergy /= 1e12;
			
		} else if (starts_with(line, "-AccessType")) {
			tmp = parse_string_after_colon(line);
			if (tmp == "CMOS")
				accessType = CMOS_access;
			else if (tmp == "BJT")
		...
		} else if (starts_with(line, "-AccessCMOSWidth")) {
				widthAccessCMOS = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-VoltageDropAccessDevice")) {
			voltageDropAccessDevice = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-LeakageCurrentAccessDevice")) {
			leakageCurrentAccessDevice = parse_double_after_colon(line);
			leakageCurrentAccessDevice /= 1e6;
			
		} else if (starts_with(line, "-DRAMCellCapacitance")) {
			if (memCellType != DRAM && memCellType != eDRAM)
				...
				capDRAMCell = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-SRAMCellNMOSWidth")) {
			if (memCellType != SRAM)
				std::cout << "Warning: The input of SRAM cell NMOS width is ignored because the memory cell is not SRAM." << std::endl;
			else
				widthSRAMCellNMOS = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-SRAMCellPMOSWidth")) {
			if (memCellType != SRAM)
				std::cout << "Warning: The input of SRAM cell PMOS width is ignored because the memory cell is not SRAM." << std::endl;
			else
				widthSRAMCellPMOS = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-ReadFloating")) {
			tmp = parse_string_after_colon(line);
			if (tmp == "true")
				readFloating = true;
			else
		...
		} else if (starts_with(line, "-FlashEraseVoltage (V)")) {
			if (memCellType != SLCNAND && memCellType != MLCNAND)
				...
				flashEraseVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-FlashProgramVoltage (V)")) {
			if (memCellType != SLCNAND && memCellType != MLCNAND)
				std::cout << "Warning: The input of programming/program voltage is ignored because the memory cell is not flash." << std::endl;
			else
				flashProgramVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-FlashPassVoltage (V)")) {
			if (memCellType != SLCNAND && memCellType != MLCNAND)
				std::cout << "Warning: The input of pass voltage is ignored because the memory cell is not flash." << std::endl;
			else
				flashPassVoltage = parse_double_after_colon(line);
			
		} else if (starts_with(line, "-FlashEraseTime")) {
			if (memCellType != SLCNAND && memCellType != MLCNAND)
				std::cout << "Warning: The input of erase time is ignored because the memory cell is not flash." << std::endl;
			else {
				flashEraseTime = parse_double_after_colon(line);
				flashEraseTime /= 1e3;
			}
			
		} else if (starts_with(line, "-FlashProgramTime")) {
			if (memCellType != SLCNAND && memCellType != MLCNAND)
				std::cout << "Warning: The input of erase time is ignored because the memory cell is not flash." << std::endl;
			else {
				flashProgramTime = parse_double_after_colon(line);
				flashProgramTime /= 1e6;
			}
			
		} else if (starts_with(line, "-GateCouplingRatio")) {
			if (memCellType != SLCNAND && memCellType != MLCNAND)
				std::cout << "Warning: The input of gate coupling ratio (GCR) is ignored because the memory cell is not flash." << std::endl;
			else {
				gateCouplingRatio = parse_double_after_colon(line);
			}
			
		} else if (starts_with(line, "-MLC")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of MLC is ignored because the memory is not CAM." << std::endl;
			else {
				tmp = parse_string_after_colon(line);
				if (tmp == "true")
					camMLC = true;
		...
		} else if (starts_with(line, "-MatchCMOSWidth")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of MatchCMOSWidth is ignored because the memory is not CAM." << std::endl;
			else {
				camWidthMatchTran = parse_double_after_colon(line);
			}
			
		} else if (starts_with(line, "-NumRowPort")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of NumRowPort is ignored because the memory is not CAM." << std::endl;
			else {
				camNumRow = parse_int_after_colon(line);
				for(int i=0;i<camNumRow;i++) {
					camPort[0][i].IsCol = false;
				}
			}
			
		} else if (starts_with(line, "-RowPort:PortType")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of RowPort is ignored because the memory is not CAM." << std::endl;
			else {
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
                                replace(tmp.begin(), tmp.end(), ':', ' ');
                                std::istringstream tmp_stream(tmp);
                                tmp_stream >> index >> tmp;
		...
		} else if (starts_with(line, "-RowPort:CmosRegion")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of RowPort:CmosRegion is ignored because the memory is not CAM." << std::endl;
			else {
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
                                replace(tmp.begin(), tmp.end(), ':', ' ');
		...
		} else if (starts_with(line, "-RowPort:numCmos")) {
			if (designTarget != CAM_chip)
				...
				int temp;
                                ...
                                replace(tmp.begin(), tmp.end(), ':', ' ');
                                std::istringstream tmp_stream(tmp);
                                tmp_stream >> index >> temp;

				camPort[0][index].numCmos = temp;
			}
			
		} else if (starts_with(line, "-RowPort:widthCmos")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of RowPort:widthCmos is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-RowPort:isNMOS")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of RowPort:isNMOS is ignored because the memory is not CAM." << std::endl;
			else {
				tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
                                replace(tmp.begin(), tmp.end(), ':', ' ');
		...
		} else if (starts_with(line, "-RowPort:volSetLRS")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of RowPort:volSetLRS is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-RowPort:volReset")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of RowPort:volReset is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-RowPort:volSetMRS")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of RowPort:volSetMRS is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-RowPort:volSearch0")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of RowPort:volSearch0 is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-RowPort:volSearch1")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of RowPort:volSearch1 is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-RowPort:widthWire")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of RowPort:widthWire is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-RowPort:leak")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:widthWire is ignored because the memory is not CAM." << std::endl;
			else {
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
                                replace(tmp.begin(), tmp.end(), ':', ' ');
		...
		} else if (starts_with(line, "-NumColPort")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of NumColPort is ignored because the memory is not CAM." << std::endl;
			else {
				camNumCol = parse_int_after_colon(line);
				for(int i=0;i<camNumCol;i++) {
					camPort[1][i].IsCol = true;
				}
			}
			
		} else if (starts_with(line, "-ColPort:PortType")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort is ignored because the memory is not CAM." << std::endl;
			else {
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
                                replace(tmp.begin(), tmp.end(), ':', ' ');
		...
		} else if (starts_with(line, "-ColPort:CmosRegion")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:CmosRegion is ignored because the memory is not CAM." << std::endl;
			else {
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
                                replace(tmp.begin(), tmp.end(), ':', ' ');
		...
		} else if (starts_with(line, "-ColPort:numCmos")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:numCmos is ignored because the memory is not CAM." << std::endl;
			else {
				int temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-ColPort:widthCmos")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:widthCmos is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-ColPort:isNMOS")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:isNMOS is ignored because the memory is not CAM." << std::endl;
			else {
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
                                replace(tmp.begin(), tmp.end(), ':', ' ');
		...
		} else if (starts_with(line, "-ColPort:volSetLRS")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:volSetLRS is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-ColPort:volReset")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:volReset is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-ColPort:volSetMRS")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:volSetMRS is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-ColPort:volSearch0")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:volSearch0 is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-ColPort:volSearch1")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:volSearch1 is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
                                tmp = parse_string_after_colon(line);
                                tmp = parse_string_after_colon(tmp);
		...
		} else if (starts_with(line, "-ColPort:widthWire")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:widthWire is ignored because the memory is not CAM." << std::endl;
			else {
				double temp;
		...
		} else if (starts_with(line, "-ColPort:leak")) {
			if (designTarget != CAM_chip)
				std::cout << "Warning: The input of ColPort:widthWire is ignored because the memory is not CAM." << std::endl;
			else {
                                tmp = parse_string_after_colon(line);
