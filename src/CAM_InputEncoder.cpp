...
void CAM_InputEncoder::Initialize(TypeOfInputEncoder _typeEncoder, bool _isCustom, double _capLoad, 
        double _resLoad, std::shared_ptr<InputParameter> _inputParameter) {
	if (initialized)
...
void CAM_InputEncoder::CalculatePower() {
	if (!initialized) {
		...
		if(isCustom) {
			...
			leakage = CalculateGateLeakage(NAND, 2, widthNandN, widthNandP, inputParameter->temperature, inputParameter->tech) * inputParameter->tech->vdd * numInputBits * 2;
			leakage += outputDriver->leakage * numInputBits * 2;

			/* nand and the driver */
			double cap;
			cap = outputDriver->capInput[0] + capNandOut;
			readDynamicEnergy = numInputBits * 2 * cap * inputParameter->tech->vdd * inputParameter->tech->vdd;
