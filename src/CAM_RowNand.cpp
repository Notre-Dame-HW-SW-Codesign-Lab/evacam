...
void CAM_RowNand::Initialize(int _numRow, double _capLoad, double _resLoad,
	...
	if (numNandInput > 0) {
		...
		outputDriver->Initialize(logicEffortNand, capNand, capLoad, resLoad, driverInv, areaOptimizationLevel, minDriverCurrent, inputParameter);
		// end_change
	} else {
		...
		outputDriver->Initialize(1, capInv, capLoad, resLoad, driverInv, areaOptimizationLevel, minDriverCurrent, inputParameter);
		// end_change
	}
