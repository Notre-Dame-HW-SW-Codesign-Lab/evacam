...
#include "../include/CAM_CustomSA.h"
...
*/
void CAM_SenseAmp::Initialize(long long _numColumn, TypeOfSenseAmp _typeSA, bool _isCustom, double _senseVoltage, double _pitchSenseAmp, std::string _fileCustomSA, std::shared_ptr<InputParameter> _inputParameter) {
	if (initialized)
	...
	normalSenseAmp->Initialize(numColumn, typeSA == nvsim_current_sense, senseVoltage, pitchSenseAmp, inputParameter);
	...
	if(isCustom) {
		...
		while (fscanf(fp, "%[^\n]\n", line) != EOF) {
			...
			if (!strncmp("-Height", line, strlen("-Height"))) {
				...
				customSA->height = tmp * inputParameter->tech->featureSize;
				continue;
			}
			if (!strncmp("-Width", line, strlen("-Width"))) {
				sscanf(line, "-Width (F): %lf", &tmp);
				customSA->width = tmp * inputParameter->tech->featureSize;
				continue;
			}
