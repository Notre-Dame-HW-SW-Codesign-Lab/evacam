...
void CAM_BasicMMR::Initialize(int _numInputBits, double _capLoad, double _resLoad, 
	...
	if(numInputBits == 8) {
		...
		CalculateGateCapacitance(NOR, 8, widthN*2, 0, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp1, &capLAintra);
		CalculateGateCapacitance(NOR, 4, widthN*2, 0, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp2, &capLAout);
		CalculateGateCapacitance(INV, 1, widthN*3, widthP, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp, capD);
		CalculateGateCapacitance(INV, 1, widthN*4, widthP, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp, capD+1);
		CalculateGateCapacitance(INV, 1, widthN*5, widthP, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp, capD+2);
		CalculateGateCapacitance(INV, 1, widthN*6, widthP, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp3, capD+3);
		CalculateGateCapacitance(INV, 1, widthN, widthP, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &capInvIn, &capInvOut);
		double logicEffort = 2 / (1+inputParameter->tech->pnSizeRatio);
		LookAheadDriver.Initialize(logicEffort, capLAout, capLaLoad, resLaLoad, true, latency_first, 0, inputParameter);
...
void CAM_BasicMMR::CalculateArea(){
	if (!initialized) {
		...
		if(numInputBits == 8) {
			...
			CalculateGateArea(INV, 1, widthN, widthP, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &hInv, &wInv, inputParameter->UseUpdatedLib);
			// the LA for output, dynamic logic for 8-input NOR

			CalculateGateArea(NOR, 8, widthN*2, 0, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &hLAout, &wLAout, inputParameter->UseUpdatedLib);
			hLAout += hInv;
			wLAout =MAX(wLAout, wInv);
			// the LA for internal, dynamic logic for 4-input NOR
			CalculateGateArea(NOR, 4, widthN*2, 0, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &hLAintra, &wLAintra, inputParameter->UseUpdatedLib);
			hLAintra += hInv;
			wLAintra =MAX(wLAintra, wInv);
			// the mem_data line: 2/3/4/5-input NAND dynamic logic
			for(int i=0;i<4;i++){
				CalculateGateArea(NAND, i+2, widthN*(i+3), 0, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, hD+i, wD+i, inputParameter->UseUpdatedLib);
				hD[i] += hInv;
				wD[i] =MAX(wD[i], wInv);
				area += (hD[i]*wD[i]);
			}
...
void CAM_BasicMMR::CalculateRC() {
	if (!initialized) {
		...
		if(numInputBits == 8) {
			...
			double tmp;
			// footed dynamic logic, Nmos sized as 2 in NOR, n+1 in NAND
			CalculateGateCapacitance(NOR, 8, widthN*2, 0, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp, &capLAintra);
			CalculateGateCapacitance(NOR, 4, widthN*2, 0, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp, &capLAout);
			CalculateGateCapacitance(INV, 1, widthN*3, widthP, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp, capD);
			CalculateGateCapacitance(INV, 1, widthN*4, widthP, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp, capD+1);
			CalculateGateCapacitance(INV, 1, widthN*5, widthP, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp, capD+2);
			CalculateGateCapacitance(INV, 1, widthN*6, widthP, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &tmp, capD+3);
			CalculateGateCapacitance(INV, 1, widthN, widthP, inputParameter->tech->featureSize*MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &capInvIn, &capInvOut);
		} else {
			// TODO 4-input and 2-input MMR
			std::cout << "[CAM_MMR] Error: Only support 8-input MMR block by now!" << std::endl;
...
void CAM_BasicMMR::CalculateLatency(double _rampInput) {
	if (!initialized) {
		...
		if(numInputBits == 8) {
	    	...
	    	gm = CalculateTransconductance(widthN*2, NMOS, inputParameter->tech);
	    	...
	    	// inverter part
	    	resPullDown = CalculateOnResistance(widthN, NMOS, inputParameter->temperature, inputParameter->tech);
	    	gm = CalculateTransconductance(widthN, NMOS, inputParameter->tech);
	    	beta = 1 / (resPullDown * gm);
	    	...
	    	// dynamic logic part
	    	resPullDown = CalculateOnResistance(widthN*(4+1), NMOS, inputParameter->temperature, inputParameter->tech);
	    	gm = CalculateTransconductance(widthN*(4+1), NMOS, inputParameter->tech);
	    	beta = 1 / (resPullDown * gm);
	    	cap = capD[3] + capLoad;
	    	tr = resPullDown * cap;
...
void CAM_BasicMMR::CalculatePower() {
	if (!initialized) {
		...
		if(numInputBits == 8) {
			...
			double leakageInv = CalculateGateLeakage(INV, 1, widthN, widthP, inputParameter->temperature, inputParameter->tech);
			// TODO: double check
			leakage += (leakageP*(1+1+8) + leakageInv*(6+2)) * inputParameter->tech->vdd;
	    	...
	    	cap = capD[3] + capLoad;
			readDynamicEnergy += (cap * inputParameter->tech->vdd * inputParameter->tech->vdd * 8);
