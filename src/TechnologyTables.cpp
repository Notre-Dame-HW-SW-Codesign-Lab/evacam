...
TechnologySpec BuildTechnologySpec(int _featureSizeInNano, DeviceRoadmap _deviceRoadmap, bool _UseUpdatedLib) {
    TechnologySpec spec{};
    double currentOnNmosRaw[101]{};
    double currentOnPmosRaw[101]{};
    double currentOffNmosRaw[101]{};
    double currentOffPmosRaw[101]{};

#define featureSizeInNano spec.featureSizeInNano
#define featureSize spec.featureSize
#define deviceRoadmap spec.roadmap
#define UseUpdatedLib spec.useUpdatedLib
#define vdd spec.vdd
#define vth spec.vth
#define vdsatNmos spec.vdsatNmos
#define vdsatPmos spec.vdsatPmos
#define phyGateLength spec.phyGateLength
#define capIdealGate spec.capIdealGate
#define capFringe spec.capFringe
#define capJunction spec.capJunction
#define capOverlap spec.capOverlap
#define capSidewall spec.capSidewall
#define capDrainToChannel spec.capDrainToChannel
#define capOx spec.capOx
#define buildInPotential spec.buildInPotential
#define effectiveElectronMobility spec.effectiveElectronMobility
#define effectiveHoleMobility spec.effectiveHoleMobility
#define pnSizeRatio spec.pnSizeRatio
#define effectiveResistanceMultiplier spec.effectiveResistanceMultiplier
#define currentOnNmos currentOnNmosRaw
#define currentOnPmos currentOnPmosRaw
#define currentOffNmos currentOffNmosRaw
#define currentOffPmos currentOffPmosRaw
#define capPolywire spec.capPolywire
#define currentGmNmos spec.currentGmNmos
#define currentGmPmos spec.currentGmPmos
#define heightFin spec.heightFin
#define widthFin spec.widthFin
#define PitchFin spec.PitchFin
    ...
    if (UseUpdatedLib) {
        ...
        } else if (_featureSizeInNano == 10) {	/* TODO: actually 36nm */
            if (_deviceRoadmap == HP) {
        ...
        if (_featureSizeInNano >= 200) { //TODO : only for test
            if (_deviceRoadmap == HP) {
                vdd = 1.5;
                vth = 300e-3;
        ...
        } else if (_featureSizeInNano >= 120) {	/* TODO: actually 100nm */
            if (_deviceRoadmap == HP) {
            ...
            if (_deviceRoadmap == HP) {
                ...
                /* PMOS off current, Unit: A/m. TODO: set PMOS = NMOS because MASTAR value is obviously wrong */
        ...
        } else if (_featureSizeInNano >= 32) {	/* TODO: actually 36nm */
            if (_deviceRoadmap == HP) {
                ...
                currentOffPmos[0] = currentOffNmos[0];	/* TODO: set PMOS = NMOS because MASTAR value is obviously wrong */
                currentOffPmos[10] = currentOffNmos[10];
                currentOffPmos[20] = currentOffNmos[20];
                currentOffPmos[30] = currentOffNmos[30];
                currentOffPmos[40] = currentOffNmos[40];
                ...
                currentOnPmos[0] = 878.6;  //TODO currentOnPmos even larger than currentOnNmos ?
            ...
            if (_deviceRoadmap == HP) {
                /* MASTAR 5 - hp-soi-2015.pro */ /* TODO: actually 25nm */
                vdd = 0.9;
                vth = 128.72e-3;
                ...
                currentOnPmos[0] = currentOnNmos[0] / 2; /* TODO: MASTER values are absolutely wrong so that CACTI method is temporarily used here */
                ...
                currentOffNmos[0] = 1.52e-7 * 3.93e6; /* TODO: MASTER values are absolutely wrong so that mem_data are translated from 32nm */
                ...
                currentOnPmos[90] = currentOnNmos[80] / 2;
                currentOnPmos[100] = currentOnNmos[90] / 2;
                currentOffNmos[0] = 3.02e-5 / 1.86; /* TODO: MASTER values are absolutely wrong so that mem_data are translated from 32nm */
    ...
    capPolywire = 0.0;	/* TODO: we need to find the values */
#undef featureSizeInNano
#undef featureSize
#undef deviceRoadmap
#undef UseUpdatedLib
#undef vdd
...
#undef capOverlap
#undef capSidewall
#undef capDrainToChannel
#undef capOx
#undef buildInPotential
#undef effectiveElectronMobility
#undef effectiveHoleMobility
#undef pnSizeRatio
#undef effectiveResistanceMultiplier
#undef currentOnNmos
#undef currentOnPmos
#undef currentOffNmos
#undef currentOffPmos
    spec.currentOnNmos = CollectTemperatureAnchors(currentOnNmosRaw);
    spec.currentOnPmos = CollectTemperatureAnchors(currentOnPmosRaw);
    spec.currentOffNmos = CollectTemperatureAnchors(currentOffNmosRaw);
    spec.currentOffPmos = CollectTemperatureAnchors(currentOffPmosRaw);
    return spec;
#undef capPolywire
#undef currentGmNmos
#undef currentGmPmos
...
static constexpr std::array<TechnologyKey, 24> kSupportedTechnologyKeys{{
    {45, HP, true},
    {45, LP, true},
    {45, FEFET, true},
    {45, FEFET, false},
...
std::array<TechnologySpec, kSupportedTechnologyKeys.size()> BuildTechnologySpecs() {
    std::array<TechnologySpec, kSupportedTechnologyKeys.size()> specs{};
    for (size_t i = 0; i < kSupportedTechnologyKeys.size(); i++) {
        const TechnologyKey &key = kSupportedTechnologyKeys[i];
        specs[i] = BuildTechnologySpec(key.featureSizeInNano, key.roadmap, key.useUpdatedLib);
    }
    return specs;
}

const std::array<TechnologySpec, kSupportedTechnologyKeys.size()> kTechnologySpecs = BuildTechnologySpecs();
...
const TechnologySpec *FindTechnologySpec(
        ...
        DeviceRoadmap roadmap,
        bool useUpdatedLib) {
    for (const auto &spec : kTechnologySpecs) {
    ...
    if (!useUpdatedLib) {
        ...
        if (bucketNode != -1) {
            for (const auto &spec : kTechnologySpecs) {
                if (spec.featureSizeInNano == bucketNode &&
                        spec.roadmap == roadmap &&
                        spec.useUpdatedLib == useUpdatedLib) {
                    return &spec;
                }
