...
void CAM_MMR::Initialize(int _numInputBits, BufferDesignTarget _areaOptimizationLevel, 
	...
	CAM_BasicMMR tmp;
	tmp.Initialize(8, 0, 0, 0, 0, inputParameter);
	double logicEffort = 6 / (1+inputParameter->tech->pnSizeRatio);
	outputDriver.Initialize(logicEffort, tmp.capD[3], capLoad, resLoad, false, areaOptimizationLevel, 0, inputParameter);
	...
	BasicMMR.Initialize(8, outputDriver.capInput[0], 0, tmp.capInvIn*3 * 3, 0, inputParameter /* TODO the gate resistance as zero*/);
	initialized = true;
        CalculateArea();
        CalculateRC();
        CalculatePower();
