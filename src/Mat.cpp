...
void Mat::Initialize(int _numRowSubarray, int _numColumnSubarray, int _numAddressBit, long _numDataBit,
	...
	if (initialized)
		PRINT_VERBOSE("[Mat] Warning: Already initialized!");
	...
	if (_numActiveSubarrayPerRow > numColumnSubarray) {
		PRINT_VERBOSE("[Mat] Warning: The number of active subarray per row is larger than the number of subarray per row!");
	...
	if (_numActiveSubarrayPerColumn > numRowSubarray) {
		PRINT_VERBOSE("[Mat] Warning: The number of active subarray per column is larger than the number of subarray per column!");
	...
	if (_numAddressBit <= 0) {
		...
		invalid = true;
		PRINT_VERBOSE("[Mat]: number of address bit error.");
		initialized = true;
		return;
	}
	...
	if (numRow < 16) {
		invalid = true;
		PRINT_VERBOSE("[Mat]: Word width is impractically small.");
		initialized = true;
		return;
	} else if (numRow > 512) {
		invalid = true;
		PRINT_VERBOSE("[Mat]: Word width is impractically large.");
		initialized = true;
		return;
	}
	// TODO
	...
	if (numColumn < 8) {
		invalid = true;
		PRINT_VERBOSE("[Mat]: Column width is too small.");
		initialized = true;
		return;
	} else if (numColumn > 512) {
		invalid = true;
		PRINT_VERBOSE("[Mat]: Column width is too big.");
		initialized = true;
		return;
	}
	...
	subarray->Initialize(
                ...
                inputParameter->withInputEnc, 
                inputParameter->typeInputEnc,  
                inputParameter->customInputEnc,
		inputParameter->typeSenseAmp,  
                inputParameter->customSenseAmp, 
                inputParameter->withWriteDriver,
		inputParameter->withOutputAcc, 
                inputParameter->withPriorityEnc,  
                (BufferDesignTarget)CAM_opt->Proirity,
                inputParameter->withInputBuffer, 
                inputParameter->withOutputBuffer, 
                inputParameter->cell->camType, 
                inputParameter->searchFunction, 
                inputParameter->cell->withVariation, 
                inputParameter,
                localWire,
                CAM_opt);
	...
	if (subarray->invalid) {
		invalid = true;
		PRINT_VERBOSE("[Mat]: Subarray is invalid.");
		initialized = true;
		return;
	}
	...
	if (numAddressRowPredecoderBlock1 < 0) {
		invalid = true;
		PRINT_VERBOSE("numAddressRowPrecoderBlock1 < 0");
		initialized = true;
		return;
	}
	...
	rowPredecoderBlock1->Initialize(numAddressRowPredecoderBlock1, capLoadRowPredecoder, 0 /* TODO */, inputParameter);
	rowPredecoderBlock2->Initialize(numAddressRowPredecoderBlock2, capLoadRowPredecoder, 0 /* TODO */, inputParameter);
	...
	bitlineMuxPredecoderBlock1->Initialize(numAddressBitlineMuxPredecoderBlock1, capLoadMuxPredecoder, 0 /* TODO */, inputParameter);
	bitlineMuxPredecoderBlock2->Initialize(numAddressBitlineMuxPredecoderBlock2, capLoadMuxPredecoder, 0 /* TODO */, inputParameter);
	...
	senseAmpMuxLev1PredecoderBlock1->Initialize(numAddressSenseAmpMuxLev1PredecoderBlock1, capLoadMuxPredecoder, 0 /* TODO */, inputParameter);
	senseAmpMuxLev1PredecoderBlock2->Initialize(numAddressSenseAmpMuxLev1PredecoderBlock2, capLoadMuxPredecoder, 0 /* TODO */, inputParameter);
	...
	senseAmpMuxLev2PredecoderBlock1->Initialize(numAddressSenseAmpMuxLev2PredecoderBlock1, capLoadMuxPredecoder, 0 /* TODO */, inputParameter);
	senseAmpMuxLev2PredecoderBlock2->Initialize(numAddressSenseAmpMuxLev2PredecoderBlock2, capLoadMuxPredecoder, 0 /* TODO */, inputParameter);
