...
BankWithHtree::BankWithHtree(const BankWithHtree& old_bank) {
       ...
       /*
       numHorizontalAddressBitToRoute = NULL;   
       numHorizontalDataDistributeBitToRoute = NULL;
       numHorizontalDataBroadcastBitToRoute = NULL;
       numHorizontalWire = NULL;		 
       numSumHorizontalWire = NULL;
       numActiveHorizontalWire = NULL;	
       lengthHorizontalWire = NULL;	
       numVerticalAddressBitToRoute = NULL; 
       numVerticalDataDistributeBitToRoute = NULL;
       numVerticalDataBroadcastBitToRoute = NULL; 
       numVerticalWire = NULL;			 
       numSumVerticalWire = NULL;		 
       numActiveVerticalWire = NULL;	
...
void BankWithHtree::Initialize(int _numRowMat, int _numColumnMat, long long _capacity,
	...
	if (!_internalSenseAmp) {
		invalid = true;
		PRINT_VERBOSE("[Bank] Htree organization does not support external sense amplification scheme");
		return;
	}
	if (initialized)
		PRINT_VERBOSE("[Bank] Warning: Already initialized!");
	...
	if (_numActiveMatPerRow > numColumnMat) {
		PRINT_VERBOSE("[Bank] Warning: The number of active subarray per row is larger than the number of subarray per row!");
		PRINT_VERBOSE(_numActiveMatPerRow << " > " << numColumnMat);
		numActiveMatPerRow = numColumnMat;
	} else {
		numActiveMatPerRow = _numActiveMatPerRow;
	}
	if (_numActiveMatPerColumn > numRowMat) {
		PRINT_VERBOSE("[Bank] Warning: The number of active subarray per column is larger than the number of subarray per column!");
		PRINT_VERBOSE(_numActiveMatPerColumn << " > " << numRowMat);
	...
	if (_numActiveSubarrayPerRow > numColumnSubarray) {
		PRINT_VERBOSE("[Bank] Warning: The number of active subarray per row is larger than the number of subarray per row!");
		PRINT_VERBOSE(_numActiveSubarrayPerRow << " > " << numColumnSubarray);
	...
	if (_numActiveSubarrayPerColumn > numRowSubarray) {
		PRINT_VERBOSE("[Bank] Warning: The number of active subarray per column is larger than the number of subarray per column!");
		PRINT_VERBOSE(_numActiveSubarrayPerColumn << " > " << numRowSubarray);
	...
	while (h > 0) {
		if (numDataDistributeBitToRoute + numDataBroadcastBitToRoute == 0 || numAddressBitToRoute == 0) {
			invalid = true;
			PRINT_VERBOSE("Reduce H an V to zero");
			initialized = true;
			return;
		}
		/* If there is possibility to reduce the mem_data bits */
		...
		if (numDataDistributeBitToRoute + numDataBroadcastBitToRoute == 0 || numAddressBitToRoute == 0) {
			invalid = true;
			PRINT_VERBOSE("numDataDistributeBitToRoute + numDataBroadcastBitToRoute == 0 || numAddressBitToRoute == 0");
			initialized = true;
			return;
		}
		/* If there is possibility to reduce the mem_data bits on vertical */
	...
	if (numDataDistributeBitToRoute + numDataBroadcastBitToRoute == 0 || numAddressBitToRoute == 0) {
		invalid = true;
		PRINT_VERBOSE("numDataDistributeBitToRoute" << std::endl);
		initialized = true;
		return;
	}
	if (columnToActive > 1) {
	...
	if (memoryType == mem_data) {
		if (numRowPerSet > (int)pow(2, numDataBroadcastBitToRoute)) {
			...
			invalid = true;
			PRINT_VERBOSE("no multiple rows");
			initialized = true;
			return;
		}
	}
	...
	mat->Initialize(numRowSubarray, numColumnSubarray, numAddressBitToRoute, matBlockSize,
			...
			muxSenseAmp, internalSenseAmp, muxOutputLev1, muxOutputLev2, areaOptimizationLevel, 
                        memoryType, camType, searchFunction, inputParameter, localWire, CAM_opt);

	/* Check if mat is under a legal configuration */
	if (mat->invalid) {
		invalid = true;
		PRINT_VERBOSE("[Bank] invalid mat configurations!");
		initialized = true;
		return;
	}
	...
	if (!initialized) {
		...
		if (memoryType == mem_data)
			if (height / width > CONSTRAINT_ASPECT_RATIO_BANK || width / height > CONSTRAINT_ASPECT_RATIO_BANK) {
				/* illegal */
				invalid = true;
				PRINT_VERBOSE("aspect ratio doesn't meet the constraint");
				PRINT_VERBOSE("height / width = " << height / width << "|| width / height = " << width / height);
				height = width = area = 1e41;
				return;
			}
...
void BankWithHtree::CalculateRC() {
	if (!initialized) {
		...
		//mat->CalculateRC();
                ...
                PRINT_VERBOSE("[Bank] Error: Cannot calculate RC while initialized and invalid.");
        }
}
		...
		if (inputParameter->designTarget == CAM_chip) {
			if (inputParameter->NoPrechargeInc) {
				...
				if (inputParameter->withOutputAcc || mat->muxSenseAmp > 1) {
					PRINT_VERBOSE("[Bank] Warning: Bit serial or Mux on SA design, but latency only shows a single sense.");
				}
			} else {
