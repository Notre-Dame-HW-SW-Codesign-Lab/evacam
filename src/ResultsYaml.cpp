...
void write_breakdown(YamlWriter& y, const Result& result) {
    const auto& input = result.inputParameter;
...
void write_summary(YamlWriter& y, const Result& result) {
    ...
    y.end_map();
    double mat_area_pct = input->cell->area * input->tech->featureSize * input->tech->featureSize *
                          bank->capacity / bank->numRowMat / bank->numColumnMat / bank->mat->area * 100;
    double subarray_area_pct = input->cell->area * input->tech->featureSize * input->tech->featureSize *
                               bank->capacity / bank->numRowMat / bank->numColumnMat / bank->numRowSubarray /
