...
void CAM_Encoder::Initialize(int _numInputBit, BufferDesignTarget _areaOptimizationLevel, 
	...
	outputDriver.Initialize(logicEffort, capInvOutput, capLoad, resLoad, false, areaOptimizationLevel, 0, inputParameter);

	widthNorN = MIN_NMOS_SIZE * inputParameter->tech->featureSize;
	widthNorP = inputParameter->tech->pnSizeRatio * MIN_NMOS_SIZE * inputParameter->tech->featureSize;
	...
	BasicEncoder.Initialize(8, capNorInput, 0, inputParameter /*TODO gate resistance */);
	initialized = true;
        CalculateArea();
        CalculateRC();
        CalculatePower();
