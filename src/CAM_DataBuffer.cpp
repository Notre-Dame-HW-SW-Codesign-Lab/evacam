...
*/
void CAM_DataBuffer::Initialize(bool _differential, double _capLoad, double _resLoad, std::shared_ptr<InputParameter> _inputParameter) {
	if (initialized)
	...
	outputDriver->Initialize(logicEffort, capNandIn+capNandOut, capLoad, resLoad, false, latency_first, 0, inputParameter);
	initialized = true;
        CalculateArea();
        CalculateRC();
        CalculatePower();
...
void CAM_DataBuffer::CalculatePower() {
	if (!initialized) {
		...
		leakage = CalculateGateLeakage(NAND, 2, widthNandN, widthNandP, inputParameter->temperature, inputParameter->tech) * inputParameter->tech->vdd *4;
