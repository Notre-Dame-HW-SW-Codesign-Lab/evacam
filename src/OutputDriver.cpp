...
void OutputDriver::Initialize(double _logicEffort, double _inputCap, double _outputCap, double _outputRes,
	...
	double minNMOSDriverWidth = minDriverCurrent / inputParameter->tech->currentOnNmos[inputParameter->temperature - 300];
	minNMOSDriverWidth = MAX(MIN_NMOS_SIZE * inputParameter->tech->featureSize, minNMOSDriverWidth);

	if (minNMOSDriverWidth > inputParameter->maxNmosSize * inputParameter->tech->featureSize) {
		invalid = true;
	...
	if (areaOptimizationLevel == latency_first) {
		...
		if (optimalNumStage > MAX_INV_CHAIN_LEN) {/* Exceed maximum stages */
			if (WARNING)
				PRINT_VERBOSE("[WARNING] Exceed maximum inverter chain length!");
		...
		widthNMOS[optimalNumStage-1] = MAX(MIN_NMOS_SIZE * inputParameter->tech->featureSize,
				inputCapLast / CalculateGateCap(1/*meter*/, inputParameter->tech) / (1.0 + inputParameter->tech->pnSizeRatio));

		if (widthNMOS[optimalNumStage-1] > inputParameter->maxNmosSize * inputParameter->tech->featureSize) {
			if (WARNING)
				std::cout << "[WARNING] Exceed maximum NMOS size!" << std::endl;
			widthNMOS[optimalNumStage-1] = inputParameter->maxNmosSize * inputParameter->tech->featureSize;
			/* re-Calculate the logic effort */
			double capLastStage = CalculateGateCap((1 + inputParameter->tech->pnSizeRatio) * inputParameter->maxNmosSize * inputParameter->tech->featureSize, inputParameter->tech);
		...
		if (widthNMOS[optimalNumStage-1] < minNMOSDriverWidth) {
			...
			widthPMOS[optimalNumStage-1] = widthNMOS[optimalNumStage-1] * inputParameter->tech->pnSizeRatio;

			for (int i = optimalNumStage-2; i >= 0; i--) {
				widthNMOS[i] = widthNMOS[i+1] / f;
				if (widthNMOS[i] < MIN_NMOS_SIZE * inputParameter->tech->featureSize) {
					if (WARNING)
						PRINT_VERBOSE("[WARNING] Exceed minimum NMOS size!");
					widthNMOS[i] = MIN_NMOS_SIZE * inputParameter->tech->featureSize;
				}
				widthPMOS[i] = widthNMOS[i] * inputParameter->tech->pnSizeRatio;
			}
		}
	}

	if (areaOptimizationLevel == latency_area_trade_off){
		double newOutputCap = CalculateGateCap(minNMOSDriverWidth, inputParameter->tech) * (1.0 + inputParameter->tech->pnSizeRatio);
		...
		widthPMOS[optimalNumStage] = widthNMOS[optimalNumStage] * inputParameter->tech->pnSizeRatio;
		...
		optimalNumStage = 1;
		numStage = 1;
		widthNMOS[optimalNumStage - 1] = MAX(MIN_NMOS_SIZE * inputParameter->tech->featureSize, minNMOSDriverWidth);
		if (widthNMOS[optimalNumStage - 1] > AREA_OPT_CONSTRAIN * inputParameter->maxNmosSize * inputParameter->tech->featureSize) {
			invalid = true;
		...
		widthPMOS[optimalNumStage - 1] = widthNMOS[optimalNumStage - 1] * inputParameter->tech->pnSizeRatio;
            ...
            //for now picked to ingore and hard code if it doesn't exist
                    //widthNMOS[0] = inputParameter->tech->featureSize;
        }
...
void OutputDriver::CalculateArea() {
	if (!initialized) {
		...
		for (int i = 0; i < numStage; i++) {
			CalculateGateArea(INV, 1, widthNMOS[i], widthPMOS[i], inputParameter->tech->featureSize*40, inputParameter->tech, &h, &w, inputParameter->UseUpdatedLib);
...
void OutputDriver::CalculateRC() {
	if (!initialized) {
		...
		for (int i = 0; i < numStage; i++) {
			CalculateGateCapacitance(INV, 1, widthNMOS[i], widthPMOS[i], inputParameter->tech->featureSize * MAX_TRANSISTOR_HEIGHT, inputParameter->tech, &(capInput[i]), &(capOutput[i]));
		}
	}
}
...
void OutputDriver::CalculateLatency(double _rampInput) {
	if (!initialized) {
		...
		for (int i = 0; i < numStage - 1; i++) {
			resPullDown = CalculateOnResistance(widthNMOS[i], NMOS, inputParameter->temperature, inputParameter->tech);
			capLoad = capOutput[i] + capInput[i+1];
			tr = resPullDown * capLoad;
			gm = CalculateTransconductance(widthNMOS[i], NMOS, inputParameter->tech);
		...
		/* Last level inverter */
                ...
                if (numStage != 0) {
		        resPullDown = CalculateOnResistance(widthNMOS[/*tmpN*/numStage-1], NMOS, inputParameter->temperature, inputParameter->tech);
		        capLoad = capOutput[/*tmpN*/numStage-1] + outputCap;
                        ...
                        gm = CalculateTransconductance(widthNMOS[/*tmpN*/numStage-1], NMOS, inputParameter->tech);
                        beta = 1 / (resPullDown * gm);
