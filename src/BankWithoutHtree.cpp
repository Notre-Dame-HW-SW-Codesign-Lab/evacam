...
void BankWithoutHtree::Initialize(int _numRowMat, int _numColumnMat, long long _capacity,
	...
	if (!internalSenseAmp) {
		...
		if (inputParameter->cell->memCellType == SRAM) {
			...
			voltageSense = true;
		} else if (inputParameter->cell->memCellType == MRAM || inputParameter->cell->memCellType == PCRAM || inputParameter->cell->memCellType == memristor || inputParameter->cell->memCellType == FBRAM || inputParameter->cell->memCellType == FEFETRAM) {
		...
		globalSenseAmp->Initialize(numSenseAmp, !voltageSense, senseVoltage, mat->width * numColumnMat / numSenseAmp, inputParameter);
		...
		globalBitlineMux->Initialize(numRowMat * numColumnMat / numActiveMatPerColumn / numActiveMatPerRow, numSenseAmp, globalSenseAmp->capLoad, globalSenseAmp->capLoad, 0, inputParameter);
	...
	if (!initialized) {
		...
		for (int i = 0; i < numRowMat; i++) {
			...
			if (internalSenseAmp) {
				...
				if (inputParameter->cell->memCellType == SRAM) {
					double vpre = inputParameter->cell->readVoltage;	/* This value should be equal to resetVoltage and setVoltage for SRAM */
					if (i == 0) {
					...
					if (i <  numActiveMatPerColumn) {
						energy = capGlobalBitline * inputParameter->tech->vdd * inputParameter->tech->vdd * numAddressBitRouteToMat;
					...
					double vWrite = MAX(fabs(inputParameter->cell->resetVoltage), fabs(inputParameter->cell->setVoltage));
					...
					if (i == 0) {
						...
						if (inputParameter->cell->readMode == false) {	/* current-sensing */
							...
							if (inputParameter->cell->readVoltage == 0) {  /* Current-in voltage sensing */
								...
								resLocalBitline += inputParameter->cell->resistanceOff - inputParameter->cell->resistanceOn;
								tau = resLocalBitline * capGlobalBitline + (resLocalBitline + resGlobalBitline) * capGlobalBitline / 2;
					...
					if (i <  numActiveMatPerColumn) {
						...
						if (inputParameter->cell->readMode) { /*Voltage-in voltage sensing */
